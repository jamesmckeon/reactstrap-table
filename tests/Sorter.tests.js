  ...
  describe("Sort", () => {
    it("sorts string ascending", () => {
      ...
      for (var i = 1; i < result.length; i++) {
        ...
        expect(current).isAfter(previous);
        previous = current;
      }
    });
    it("sorts string descending", () => {
      ...
      for (var i = 1; i < result.length; i++) {
        const current = result[i][fieldName];
        expect(current).isBefore(previous);
        previous = current;
      }
    });
  });
...
expect.extend({
  isAfter(received: string | number, argument: string | number) {
    const pass = received > argument;
    return { pass: pass };
  },
  isBefore(received: string | number, argument: string | number) {
    const pass = received < argument;
    return { pass: pass };
  }
});
